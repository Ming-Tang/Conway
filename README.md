...
| `w^p`| `mono1(p)` | Monomial (given coefficient of 1 and power `p`) |
| `w^p.c`| `mono(c, p)` | Monomial (given coefficient `c` and power `p`) |
...
|--|--|--|
| `a = b` | `eq(a, b)` | Equals |
| `a != b` | `ne(a, b)` | Not equals |
| `a > b` | `gt(a, b)` | Greater than |
| `a >= b` | `ge(a, b)` | Greater than or equals |
| `a < b` | `lt(a, b)` | Less than |
| `a <= b` | `le(a, b)` | Less than or equals |
| `compare(a, b)` | `compare(a, b)` | [`Array#sort`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/sort#description) comparator |
...
| `r = w` | `isUnit(a)` | `a` equals omega? |
| `r > R` | `isAboveReals(a)` | Is `a` infinite (or `inf` is positive)?
| `r < -R` | `isBelowNegativeReals(a)` | Is `-a` infinite (or `inf` is negative)?
| `a in Ord` | `isOrdinal(a)` | `a` represents an ordinal number? |
...
| `a * b` | `ordinalMult(a, b)` | [Ordinal multiplication](https://en.wikipedia.org/wiki/Ordinal_arithmetic#Multiplication) |
| `a / b` | `ordinalDivRem(b, a)` | Ordinal division and remainder (solution to `b . quotient + remainder`) |
| `a ^ p` | `ordinalPow(a, p)` | [Ordinal exponentiation](https://en.wikipedia.org/wiki/Ordinal_arithmetic#Exponentiation) |
| `a (+) b` | `add(a, b)` | [Natural sum](https://en.wikipedia.org/wiki/Ordinal_arithmetic#Natural_operations) (implemented as surreal sum) |
| `a (*) b` | `mult(a, b)` | [Natural product](https://en.wikipedia.org/wiki/Ordinal_arithmetic#Natural_operations) (implemented as surreal product) |
